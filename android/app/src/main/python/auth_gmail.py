...
class _AndroidCredentials(Credentials):
    ...
    def refresh(self, request):
        ...
        # Android-type OAuth clients have no client secret; Google's token
        # endpoint accepts a secretless refresh for them. Older tokens from
        # the retired desktop-type client still carry a secret — send it
        # only when present.
        form = {
        ...
        tokens = _java_post("https://oauth2.googleapis.com/token", form)
...
def get_account_email():
    ...
    """
    token_path = os.path.join(_get_config_dir(), "token.json")
